...
const AddRoutineModal = observer(() => {
  ...
  const handleSubmit = () => {
    if (title !== '' && startTime !== '' && endTime !== '' && !showAlert) {
      ...
      store.addRoutine({ title: title, start: startDate, end: endDate });
      setTitle('');
      setStartTime('08:00');
      setEndTime('16:00');
      setShow(false);
      ...
      <Modal show={show} onHide={() => setShow(false)}>
        ...
        <Modal.Body>
          ...
          <div className="addBreak">
            ...
            <Row
              style={{ width: '100%' }}
              className="addBreakButtonRow"
              onClick={handleAddBreak}
            >
              <Button className="addBreakButton">Add Break</Button>
            </Row>
          </div>
          {breaks.map((b) => (
            <Col className="breakCol">
              {`${
                b.description
              }, starting: ${b.start.getHours()}:${b.start.getMinutes()}, ending: ${b.end.getHours()}:${b.end.getMinutes()}`}
              <Button
                onClick={() => {
